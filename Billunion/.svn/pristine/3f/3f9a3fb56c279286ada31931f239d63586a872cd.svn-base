...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.leftTableView) {
        ...
        if (stockCell == nil) {
              CGFloat width = (self.transactionType == TransactionType_Selected) ? kLabelWidth+kLeftBtnWidth :  kLabelWidth ;
...
-(void)createLabelWith:(NSArray *)titles tableView:(UITableView *)tableView superView:(UIView *)superView
{
    ...
    for (int i = 0; i<titles.count; i++) {
        ...
        if (!label) {
            ...
            if (self.transactionType == TransactionType_Selected && tableView == self.leftTableView) {
    ...
    CGFloat width = (self.transactionType == TransactionType_Selected) ? kLabelWidth*titles.count+kLeftBtnWidth :  kLabelWidth*titles.count;
...
-(void)setTransactionType:(TransactionType)transactionType
{
    _transactionType = transactionType;
        ...
        if (_leftTableView) {
            ...
            if (transactionType == TransactionType_Selected) {
...
- (void)dealloc{
    ...
    [self.footerControl removeFromSuperview];
    NSLog(@"%s",__func__);
}
...
-(void)handleResponseWithDataArray:(NSArray *)dataArr errorStr:(NSString *)errorStr isPullUp:(BOOL)isPullUp{
    ...
    [self reloadTableViewWithDataArray:self.dataArray
                                stockView:self];
}

- (void)reloadTableViewWithDataArray:(NSArray *)array stockView:(StockView *)stockView{
    if (stockView) {
