...
-(id)initWithFrame:(CGRect)frame
{
    ...
    if(self)
    {
        ...
        _searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(15, 7, frame.size.width-10, 30)];
       ...
       _searchBar.backgroundImage = [self imageWithColor:[UIColor clearColor] size:_searchBar.bounds.size];
        [_searchBar setSearchFieldBackgroundImage:[self imageWithColor:[UIColor colorWithRGBHex:0xb0b0b0] size:_searchBar.bounds.size] forState:UIControlStateNormal];
    }
    return self;
}
...
- (void)searchBarTextDidBeginEditing:(UISearchBar *)searchBar
{
    ...
    for(UIView *view in  [[[searchBar subviews] objectAtIndex:0] subviews]) {
        if([view isKindOfClass:[NSClassFromString(@"UINavigationButton") class]]) {
            ...
            [cancel setTitleColor:[UIColor colorWithRGBHex:0xffffff] forState:UIControlStateNormal];
        }
    }
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
    ...
    if([_delegate respondsToSelector:@selector(didSearchAction:)])
    {
        [_delegate didSearchAction:searchBar.text];
    }
}
