...
- (void)requestDataCompletion:(YYRrefreshControl*)refreshControl{
    if(refreshControl == nil){
      [Hud showProgress:MBProgressHUDModeIndeterminate text:@"加载中..."];
    }
        ...
        if (errorStr == nil) {
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                ...
                for (int i = 0;i<dataArray.count;i++) {
                    ...
                    MoreIndexCollectionView *collectionView = [weakSelf getCollectionViewWithTag:(billType-1)];
...
- (void)headerRefresh:(YYRrefreshControl*)refreshControl{
    [self requestDataCompletion:refreshControl];
}
 ...
 */
-(MoreIndexCollectionView *)getCollectionViewWithTag:(NSInteger)tag
{
    UIView *view = [self.scrollView viewWithTag:(tag+collectionViewTag)];
    if ([view isKindOfClass:[UICollectionView class]]) {
        return (MoreIndexCollectionView *)view;
    }
    return nil;
}
