...
-(void)addLabels{
    self.sliderLabel = [[UILabel alloc] initWithFrame:CGRectMake(screen_width/2-20, 20, 60, 44)];
...
-(void)addNameLabels{
    self.nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(screen_width/2-40, screen_height-40, 200, 40)];
...
-(void)loadPhote:(NSInteger)index{
    ...
    if (![currentPhotoView isKindOfClass:[PhotoView class]]) {
        ...
        id mm = [self.imgArr objectAtIndex:index];
        PhotoView *photoV = nil;
        if ([mm isKindOfClass:[NSString class]]) {
             photoV = [[PhotoView alloc] initWithFrame:frame withPhotoUrl:mm];
        }else{
           photoV = [[PhotoView alloc] initWithFrame:frame withPhotoImage:mm];
        }
