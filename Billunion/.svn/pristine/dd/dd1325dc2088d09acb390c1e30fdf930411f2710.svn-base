...
@property (nonatomic,assign) float stockRange;
...
-(void)initData{
    ...
    if (self.isNearStock) {
        self.stockRange = [self.filterDict[kRange] floatValue];
    }
...
-(void)saveBtnClick
{
    ...
    }else if (self.selectedAccetors.count == 0){
        [self.filterDict removeObjectForKey:kAcceptor];
    }
    
    if (self.selectedDueTimes.count > 0) {
        self.filterDict[kDueTimeRange] = self.selectedDueTimes;
    }else if (self.selectedDueTimes.count == 0){
        [self.filterDict removeObjectForKey:kDueTimeRange];
    }

    if (self.addresses.count > 0) {
        self.filterDict[kAddress] = self.addresses;
    }else if (self.addresses.count == 0){
        [self.filterDict removeObjectForKey:kAddress];
    }
...
-(void)layoutCell2:(UITableViewCell*)cell
{
    ...
    if (self.isNearStock) {
        ...
        rangeTF.text =   [@(self.stockRange)  stringValue];
...
-(void)rightBtnClick{
    ...
    if (self.isNearStock) {
        self.stockRange = 9999999/1000.0;
    }
