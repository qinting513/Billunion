...
@property (nonatomic,strong) IsTradeView *isTradeView;
...
- (void)layoutWithType:(InfoMationViewType)type{
    ...
    if (type == InfoMationViewType_IsTrade) {
        _isTradeView = [[IsTradeView alloc] init];
        _isTradeView.isTradeViewBlock  = ^(BOOL isSureBtn){
            if (isSureBtn && weakSelf.isTradeViewBlock) {
                weakSelf.isTradeViewBlock(isSureBtn);
            }
            [weakSelf hideWithType:InfoMationViewType_IsTrade];
        };       
    }else if(type == InfoMationViewType_Delivery){
        _deliveryView = [[DeliveryView alloc] init];
        [_deliveryView.rateTF becomeFirstResponder];
        ...
        [_inPutTextView.textField becomeFirstResponder];
        _inPutTextView.inputTextViewBlock = ^(BOOL isSureBtn,NSString *message){
            if (isSureBtn && weakSelf.inputTextViewBlock) {
                if (type == InfoMationViewType_InPut_DiscountRate) {
                    if (![weakSelf.inputViewStr isEqualToString:message]) {
...
- (void)showWithType:(InfoMationViewType)type{
    ...
    }else if (type == InfoMationViewType_IsTrade){
         [self showView:_isTradeView animated:YES];
    }else{
        _inPutTextView.type = type;
        if (type == InfoMationViewType_InPut_DiscountRate) {
            _inPutTextView.placeholder = @"0.000";
...
- (void)hideWithType:(InfoMationViewType)type{
    if (type == InfoMationViewType_Delivery) {
        ...
        [self closeView:_isTradeView animated:YES];
    }else{
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _titleLabel = [UILabel labelWithText:@"是否匿名：" fontSize:17 textColor:[UIColor whiteColor] alignment:NSTextAlignmentCenter];
        [self addSubview:_titleLabel];

        _titleLabel.sd_layout.leftEqualToView(self).rightEqualToView(self).topSpaceToView(self,STRealY(30)).heightIs(STRealY(36));
        
        _contentLable = [UILabel labelWithText:nil fontSize:15 textColor:[UIColor whiteColor] alignment:NSTextAlignmentCenter];
        [self addSubview:_contentLable];
        _contentLable.sd_layout.leftEqualToView(self).rightEqualToView(self).topSpaceToView(_titleLabel,STRealY(46)).heightIs(16);
...
- (void)oKSelect:(UIButton *)btn{
    if (self.isTradeViewBlock) {
        if (btn.tag == 40) {  //取消
            self.isTradeViewBlock(NO);
        }else{
            self.isTradeViewBlock(YES);
        }
    }
}
    ...
    if (self.inputTextViewBlock) {
        if (btn.tag == 40) {  //取消
            ...
            if (self.type == InfoMationViewType_InPut_counterPartyId) {
                ...
                if ([_textField.text isPureFloat] && _textField.text.floatValue > 0 && _textField.text.floatValue <= 100 ) {
                   NSString *errorStr = [Tools judgeDiscountRateInputWithText:_textField.text];
                    if (!errorStr) {
                        self.inputTextViewBlock(YES,_textField.text);
                    }else{
...
- (void)showError:(NSString *)message{
     [[UIApplication sharedApplication].keyWindow showWarning:message];
}
