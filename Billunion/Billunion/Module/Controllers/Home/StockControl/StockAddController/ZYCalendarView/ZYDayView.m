...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        [self setImage:[UIImage imageNamed:@"calcircle"] forState:UIControlStateSelected];
...
- (void)setDate:(NSDate *)date {
    ...
    if (self.enabled) {
        ...
        if ([_manager.helper date:_date isTheSameDayThan:_manager.date] && self.enabled) {
            [self setImage:[UIImage imageNamed:@"calcircle"] forState:UIControlStateNormal];
        }
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (self.manager.dayViewBlock) {
        ...
        NSInteger seconds = [zone secondsFromGMTForDate:_date];
        NSDate *nowDate = [_date dateByAddingTimeInterval:seconds];
        NSLog(@"nowDate %@ = %@",_date, nowDate);
        self.manager.dayViewBlock(nowDate);
    }
}
