...
//                我的买入（询价买入、指定买入、指定卖出、询价卖出）
                ...
                if (self.kLineType == KLine_buyerInquiry ||
                    self.kLineType == KLine_buyerSpecify) {
                    NSInteger tradeMode = [vc.inquiryInfoVM getTradeModeWithModel:calloutView.mapModel];
                    //                    交易模式：1-挑票成交  2-整批成交
                    vc.transactionType = (tradeMode == 1) ? TransactionType_Selected : TransactionType_SelectAll;
                }
