...
-(id)initWithFrame:(CGRect)frame withPhotoUrl:(NSString*)photoUrl{
    ...
    if (self) {
        ...
        //添加图片
        self.imageView = [[UIImageView alloc] initWithFrame:CGRectMake((_scrollView.width-ViewWidth)/2, (_scrollView.height-ViewHeight)/2, ViewWidth, ViewHeight)];
...
//                        self.imageView.image = [[ShareViewModel shareInstance] getDocumentImageWithName:photoUrl];
//                    }
                }
        }else if([photoUrl isKindOfClass:[UIImage class]]){
            self.imageView.image = (UIImage *)photoUrl;
        }
